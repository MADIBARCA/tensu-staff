...
import type { StaffResponse } from '@/functions/axios/responses';
...
export const UserInfoSection: React.FC<UserInfoSectionProps> = ({ staff, onEdit }) => {
  ...
  const displayPhone = staff?.phone_number || user?.phone_number || 'Не указан';
  const photoUrl = staff?.photo_url || (user?.photo_url && typeof user.photo_url === 'string' ? user.photo_url : '') || '';
